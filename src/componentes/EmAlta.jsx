...
function EmAlta({yzfR6, filmes, setPagina}){
    ...
    return(
         <>
         {yzfR6 && !filmes && yzfR6[0] && (
    <div className="flex justify-center">
<div className="flex gap-8 p-6">
    <img 
      src={`https://image.tmdb.org/t/p/w500${yzfR6[0].poster_path}`}
      alt={yzfR6[0].title}
    ...
    <div className="flex-1">
        <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-gray-400">{yzfR6[0].release_date?.slice(0, 4)}</span>
        </div>
        <h1 className="text-white text-5xl font-bold mb-3">{yzfR6[0].title}</h1>
        <div className="flex items-center gap-2 mb-4">
            <span className="text-amber-400 text-xl">★</span>
            <span className="text-white text-xl font-semibold">{yzfR6[0].vote_average?.toFixed(1)}</span>
            <span className="text-gray-500">· {yzfR6[0].vote_count} avaliações</span>
        </div>
        <p className="text-gray-300 max-w-2xl leading-relaxed">{yzfR6[0].overview}</p>
        <button 
          className="bg-yellow-300 px-4 py-2 rounded-lg font-medium mt-4"
          onClick={() => onSeeDetailsClick(yzfR6[0])}
        >
          Ver mais
        </button>
    </div>
    ...
    {yzfR6 && !filmes && <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 p-90 pt-10 pb-15"> {yzfR6.slice(1).map((item, i) => (
        <div key={i} className="group cursor-pointer">
            <div className="relative rounded-xl ">
                <img 
                  src={`https://image.tmdb.org/t/p/w500${item.poster_path}`}
                  alt={item.title}
                  className={`w-50 aspect-[2/3] group-hover:scale-105  transition-transform rounded-xl`}
                />
                ...
                <div className="absolute top-2 right-6 bg-black/70 backdrop-blur-sm px-2 py-0.5 rounded-full flex items-center gap-1">
    ...
    {yzfR6 && !filmes && <Switch setPagina={setPagina} yzfR6={yzfR6}/>}
</>
    )
